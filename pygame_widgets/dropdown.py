...
class Dropdown(WidgetBase):
    def __init__(self, win, x, y, width, height, name, choices, **kwargs):
        ...
        if len(values) != len(choices):
            raise Exception(
                "'choices' and 'values' arguments should be indentical in size"
            )
    ...
    def listen(self, events):
        ...
        if not self._hidden:
            ...
            # Then we handle the DropdownChoice-s
    ...
    def getSelected(self):
        return self.__chosen._value if self.__chosen != None else None
...
class DropdownChoice(WidgetBase):
    ...
    def _computeBorderRadiuses(self):
        border_radius = {}
        if not self.last:
...
class HeadDropdown(DropdownChoice):
    ...
    def listen(self, events):
        ...
        if not self._hidden:
            pressed = pygame.mouse.get_pressed()[0]
            righ_click_pressed = pygame.mouse.get_pressed()[2]
            x, y = pygame.mouse.get_pos()
    ...
    @property
    def text(self):
        return self._drop.chosen.text if self._drop.chosen != None else self.__head_text
